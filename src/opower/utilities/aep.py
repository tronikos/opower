...
class AEP(ABC):
    ...
    async def async_login(
        ...
        client_url_callback  = urllib.parse.quote_plus("https:" + token_parser.token_url + "&ou-session-initiated=true", safe="")
        client_url  = urllib.parse.quote_plus(f"/x/embedded-api/redirect?client-url={client_url_callback}", safe="")
        failure_url_callback  = urllib.parse.quote_plus("https:" + token_parser.token_url + "&ou-session-initiated=true&ou-auth-error=auth-failed", safe="")
        failure_url  = urllib.parse.quote_plus(f"/x/embedded-api/redirect?client-url={failure_url_callback}", safe="")
        target = f"https://{cls.subdomain()}.opower.com/ei/app/api/authenticate" + urllib.parse.quote_plus(f"?redirectUrl={client_url}&failureUrl={failure_url}")
