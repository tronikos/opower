...
class PSE(UtilityBase):
    ...
    async def login(
        ...
        async with session.get("https://www.pse.com/api/AccountSelector/GetContractAccountJson") as resp:
        ...
        async with session.get("https://www.pse.com/en/account-and-billing/my-usage/view-my-usage") as resp:
            usage_parser.feed(await resp.text())

        session.headers.add("authorization", f"Bearer {usage_parser.opower_access_token}")
