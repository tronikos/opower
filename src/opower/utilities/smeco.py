...
class SMECO(UtilityBase):
    ...
    def supports_realtime_usage() -> bool:
        """Indicate that SMECO supports real‑time usage reads.

        The SMECO customer portal exposes 15‑minute interval data via
        Opower’s real‑time AMI API.  Returning ``True`` here enables
        clients (including Home Assistant) to call the library’s
        ``async_get_realtime_usage_reads`` method and retrieve the
        latest 15‑minute usage data.

        :returns: ``True`` to indicate that real‑time usage is available.
        """
    ...
    async def async_login(
        ...
        session.cookie_jar.clear(
            lambda cookie: cookie["domain"].endswith(".opower.com")
        )
        ...
        login_url = (
            "https://dss-smcc.opower.com"
            "/webcenter/edge/apis/identity-management-v1/cws/v1/smcc/login"
        )
